			...
			if (abstractStatements.size() == 1 && abstractStatements.get(0) instanceof StatementObject) {
				...
				if (statement instanceof ReturnStatement) {
					...
					if ((returnStatement.getExpression() instanceof SimpleName
							|| returnStatement.getExpression() instanceof FieldAccess)
				...
				if (statement instanceof ExpressionStatement) {
					...
					if (expressionStatement.getExpression() instanceof Assignment
						...
						if ((assignment.getLeftHandSide() instanceof SimpleName
								|| assignment.getLeftHandSide() instanceof FieldAccess)
...
/*	public FieldInstructionObject isCollectionAdder() {
		if (getMethodBody() != null) {
			...
			if (abstractStatements.size() == 1 && abstractStatements.get(0) instanceof StatementObject) {
				StatementObject statementObject = (StatementObject) abstractStatements.get(0);
				if (statementObject.getFieldInstructions().size() == 1
						&& statementObject.getMethodInvocations().size() == 1
						&& statementObject.getLocalVariableDeclarations().size() == 0
						&& statementObject.getLocalVariableInstructions().size() == 1
						&& this.constructorObject.parameterList.size() == 1) {
				...
				if (methodInvocation != null) {
					...
					if (methodInvocationExpression instanceof MethodInvocation) {
						...
						for (MethodDeclaration parentClassMethod : parentClassMethods) {
							if (parentClassMethod.resolveBinding()
									.isEqualTo(previousChainedMethodInvocation.resolveMethodBinding())) {
						...
						if (variableBinding.getDeclaringClass().isEqualTo(parentClass.resolveBinding()) || parentClass
								.resolveBinding().isSubTypeCompatible(variableBinding.getDeclaringClass())) {
							for (MethodInvocationObject methodInvocationObject : methodInvocations) {
								if (methodInvocationObject.getMethodInvocation().equals(methodInvocation)) {
									return methodInvocationObject;
								}
...
//					if (variableDeclaration.getVariableDeclaration().resolveBinding()
//							.isEqualTo(localVariableInstruction.getSimpleName().resolveBinding()))
//						return false;
//				}
//			}
//		}
...
//							&& parameter.getType().getArrayDimension() == 0)
//						return true;
//				}
//			}
//		}
...
//					|| fieldTypeBinding.isEqualTo(targetClassBinding)
...
/*	public Set<AbstractVariable> getDefinedFieldsThroughFields() {
	...
	public boolean equals(MethodInvocationObject mio) {
		return this.getClassName().equals(mio.getOriginClassName()) && this.getName().equals(mio.getMethodName())
				&& this.returnType.equalsClassType(mio.getReturnType())
				&& equalParameterTypes(this.constructorObject.getParameterTypeList(), mio.getParameterTypeList());
		/*
		 * this.constructorObject.getParameterTypeList().equals(mio.
	...
	public boolean equals(SuperMethodInvocationObject smio) {
		return this.getClassName().equals(smio.getOriginClassName()) && this.getName().equals(smio.getMethodName())
				&& this.returnType.equalsClassType(smio.getReturnType())
				&& equalParameterTypes(this.constructorObject.getParameterTypeList(), smio.getParameterTypeList());
		/*
		 * this.constructorObject.getParameterTypeList().equals(smio.
		 * getParameterTypeList());
		 */
	}
